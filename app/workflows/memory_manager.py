...
async def process_memory_truncation(
    ctx: Context,
    ...
    Args:
        ctx: Workflow context (used to read conversation_id and update chat_history)
        memory: ChatMemoryBuffer to process
        memory_service: MemoryService instance for summarization logic
        user_id: UUID of the authenticated user (for DB ownership checks)
    """
    ...
    messages_to_summarize, messages_to_keep = memory_service.split_messages_for_summary(
        all_messages, is_empty_truncated
    )

    try:
        summary_text = await memory_service.create_summary(conversation_id, user_id, messages_to_summarize)
        logger.info(
            "summary_created",
            conversation_id=conversation_id,
    ...
    message_ids_to_summarize = [
        ...
        if msg.additional_kwargs.get("message_id")
    ]
