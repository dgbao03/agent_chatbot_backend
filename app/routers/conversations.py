...
async def update_conversation_endpoint(
    ...
    conv = update_conversation(conversation_id, user_id, body.model_dump(exclude_unset=True), db)
    if not conv:
        raise HTTPException(status_code=404, detail="Conversation not found")
    return conv
...
async def delete_conversation_endpoint(
    ...
    deleted = delete_conversation(conversation_id, user_id, db)
    if not deleted:
        raise HTTPException(status_code=404, detail="Conversation not found")
...
async def get_messages(
    ...
    messages = load_all_messages_for_conversation(conversation_id, user_id, db)
...
async def get_active_presentation_endpoint(
    ...
    presentation_id = get_active_presentation(conversation_id, db, user_id=user_id)
